...
feign:
  services:
    order:
      ...
      auth:
        type: oauth2
        token-url: https://api.example.com/oauth/token # 默认是post json请求，参数是client_id=my-client/client_secret=my-secret/grant-type=client_credentials，x-token来自于access_token
        clients:
          - id: my-client
            secret: my-secret
```
  ...
  services:
    measure:
      ...
      auth:
        ...
        token-prefix: "Bearer "             # 默认空，设置后拼接在 header 值前面，带空格需加引号
        ...
        request-fields:                    # 自定义 Token 请求字段名（均为可选）
          client-id: client_id              # 默认 client_id
          client-secret: client_secret      # 默认 client_secret
          grant-type: grant-type            # 1.上面两项都不配置的话，则默认是grant-type以及client中grant-type也可以不配置，默认是 client_credentials； 2. 配置后 + client 也配置 grant-type 时才发送该参数
        clients:
          - id: event-client
            secret: event-secret
