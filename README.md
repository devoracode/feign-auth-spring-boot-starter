...
public interface FeignHeaderInjector {
    ...
    /**
     * 业务请求：直接操作 RequestTemplate 注入 Header。
     * 可通过 template.queries()、template.method()、template.body() 获取请求数据。
     */
    void inject(String serviceName, String requestPath, RequestTemplate template);

    /**
     * Token 请求（默认空实现）：操作 HttpHeaders 注入 Header。
     * parameters 包含 client_id、client_secret、grant_type 等 token 请求参数。
     */
...
> `inject` 用于 Feign 业务请求，`injectTokenHeaders` 用于 OAuth2 Token 请求。
> 两者独立，各自直接操作原生对象，无需适配层。
...
public class OrderSignatureInjector implements FeignHeaderInjector {
    ...
    public void inject(String serviceName, String requestPath, RequestTemplate template) {
        ...
        template.header("X-Timestamp", String.valueOf(ts));
...
public class TenantHeaderInjector implements FeignHeaderInjector {
    ...
    @Override
    public void inject(String serviceName, String requestPath, RequestTemplate template) {
        template.header("X-Tenant-Id", tenantContext.currentTenantId());
    }
}
```
